<!--section Porfolio start-->
<div id="interest">
    <div class="hand">
        < h2>
    </div>
    <div class="container-title-content">
        <h2 class="h2-red">Mes centres d'interêts</h2>
        <div class="hand align-left">
            < /h2>
        </div>
        <div class="section-content interest">
            <span class="trait">Ce que j'aime</span>
        </div>
    </div>
    <div class="hand">
        < p>
    </div>
    <p class="double-section">
    Cette section est consacrée à mes passions et à mes intérêts
    </p>
    <div class="hand">
        < /p>
    </div>

    <div class="hand">
        < article>
    </div>



    <div class="portfolio-works">

        <div class="works-item item1">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/restaurant.jpg" alt="Avatar">
                        Restaurant
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/restaurant2.png" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item2">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/music.jpg" alt="Avatar">
                        Musique
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/music2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item3">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/swim.jpg" alt="Avatar">
                        Natation
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/swim2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item4">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/esport.jpg" alt="Avatar">
                        Esport
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/esport2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item5">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/travel.jpg" alt="Avatar">
                        Voyage
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/travel2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item6">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/img.jpeg" alt="Avatar">
                        Athletisme
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/athle2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item7">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/cinema.jpg" alt="Avatar">
                        Cinéma
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/cinema2.png" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>

        <div class="works-item item8">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/mtb.jpg" alt="Avatar">
                        Cyclisme
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/mtb2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>
        </div>
        <div class="works-item item9">
            <div class="flip-card">
                <div class="flip-card-inner">
                    <div class="flip-card-front">
                        <img class="img-flip" src="./img/interest/read.jpg" alt="Avatar">
                        Lecture
                    </div>
                    <div class="flip-card-back">
                        <img class="img-flip-back" src="./img/interest/read2.jpg" alt="Avatar">
                    </div>
                </div>
            </div>

        </div>
    </div>




    <div class="hand">
        < /article>
    </div>
</div>